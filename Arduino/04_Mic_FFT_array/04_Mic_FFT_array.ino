...
Code and library reference:
  [1] Condés, E.; Homb, R.R.. "Arduino FFT libray". 2018.
      Retrieved Oktober 17, 2024 from https://github.com/kosme/arduinoFFT
*/
...
void loop() {
  ...
  // Automatically correct the offset, otherwise the graphic in python will have a spikpe of amplitude for f=0  
  for(int n=0; n<samples; n++) {
    vReal[n] = vReal[n];
  }
  ...
  // The result of each FFT is printed in the same line of the serial, according according to the method:
  // Each parameter of the line is separed by an empty space " ".
  // a. The message starts with a string "START", followed by an empty space " ".
  // b. The second part of the message is the the sampling frequency samplingFrequency, followed by an empty space " ".
  // c. The third part of the message is the dominant frequency f_peak, followed by an empty space " ".
  // d. Then each FFT-coeficient from the array vReal is printed, followed by an empty space " ".
  // e. Finally the message ends with a string "END", followed by a breakline.

  // a.
  Serial.print("START ");
  // b.
  Serial.print(samplingFrequency, 1); Serial.print(" ");
  // c.
  Serial.print(f_peak, 1); Serial.print(" ");
  // d.
  for(int n=0; n<(samples/2); n++){
    Serial.print(vReal[n], 1); Serial.print(" ");
  }
  // e.
  Serial.println("END ");
